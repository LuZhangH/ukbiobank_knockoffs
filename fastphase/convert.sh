...
DAT_DIR="/oak/stanford/groups/candes/ukbiobank"               # Location of original data
TMP_DIR="/scratch/PI/candes/ukbiobank_tmp"           # Location of intermediate files
#TMP_DIR="/home/groups/candes/scratch/ukbiobank_tmp"           # Location of intermediate files
...
# List of individuals and variants for toy example
#INDIVIDUALS="/scratch/PI/candes/ukbiobank_tmp/fastphase/individuals.txt"
#VARIANTS="/scratch/PI/candes/ukbiobank_tmp/fastphase/variants.txt"
...
########################################
# Convert HAPS.T into FBM
########################################
if [[ ! -f $BASENAME_HAP".rds" ]] || [[ $RESET -eq 1 ]]; then
  echo ""
  echo "----------------------------------------------------------------------------------------------------"
  echo "Converting haplotypes into FBM"
  echo "----------------------------------------------------------------------------------------------------"  
  Rscript --vanilla utils/hapst_to_fbm.R $CHR
else
  echo ""
  echo "----------------------------------------------------------------------------------------------------"
  echo "Skipping conversion of haplotypes into FBM because"
  echo $BASENAME_HAP".rds"" exists"
  echo "----------------------------------------------------------------------------------------------------"
fi
...
########################################
# Split INP file into train/test
########################################
if [[ ! -f $BASENAME_TRAIN".inp" ]] || [[ ! -f $BASENAME_TEST".inp" ]] || [[ $RESET -eq 1 ]]; then
  echo ""
  echo "----------------------------------------------------------------------------------------------------"
  echo "Splitting INP file into train/test files"
  echo "----------------------------------------------------------------------------------------------------"
  utils/split_inp.sh -i $BASENAME_HAP -a $BASENAME_TRAIN -e $BASENAME_TEST -n $N_TEST
else
  echo ""
  echo "----------------------------------------------------------------------------------------------------"
  echo "Skipping split of INP file into train/test files because"
  echo $BASENAME_TRAIN".inp exists"
  echo $BASENAME_TEST".inp exists"
  echo "----------------------------------------------------------------------------------------------------"
fi
...
# Mask test INP file
########################################
if [[ ! -f $BASENAME_TEST"_masked.inp" ]] || [[ $RESET -eq 1 ]]; then
  echo ""
  echo "----------------------------------------------------------------------------------------------------"
  echo "Masking test INP file"
  echo "----------------------------------------------------------------------------------------------------"
  Rscript --vanilla mask.R $CHR
else
  echo ""
  echo "----------------------------------------------------------------------------------------------------"
  echo "Skipping mask of test INP file because"
  echo $BASENAME_TEST"_masked.inp exists"
  echo "----------------------------------------------------------------------------------------------------"
fi
