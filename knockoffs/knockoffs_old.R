...
for(block in blocks) {    
    ...
    H.subjects <- H$Subject
    H <- select(H, c(Variants$Variant))
    H <- as.matrix(H)
    cat("done.\n")

    # Generate knockoff haplotypes
    cat("Generating knockoff copies... ")
    Hk <- SNPknock.knockoffHaplotypes_group(H, hmm$r, hmm$alpha, hmm$theta, Variants$Group,
                                            seed=block.start, display_progress=FALSE)    
    #Hk <- SNPknock.knockoffHaplotypes(H, hmm$r, hmm$alpha, hmm$theta, seed=block.start)
    cat("done.\n")
    ...
    # Write genotypes in PED format (experimental)
