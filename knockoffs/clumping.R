...
clumping.factor = as.numeric(args[2])/100
...
groups <- cutree(Sigma.clust, k = round(clumping.factor*nrow(Variants)))
n.groups <- max(groups)
...
cat(sprintf("Distribution of group sizes at resolution %.4f:\n",clumping.factor))
...
grp.filename <- sprintf("%s/clumping/%s%s/grp_chr%s.txt", tmp.dir, clumping.method, clumping.factor*100, chr)
...
rep.filename <- sprintf("%s/clumping/%s%s/rep_chr%s.txt", tmp.dir, clumping.method, clumping.factor*100, chr)
