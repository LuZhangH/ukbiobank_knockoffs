...
if(fit.lasso) {
  ...
  ## Extract phenotype
  phenotype <- sprintf("Y_a%s_%s", experiment.numb, fold)
  ...
  if(y.binary) {
    ...
    cat(sprintf("Fitting sparse logistic regression with %d observations and %d variables... ",
                length(y), ncol(G)))
    lasso.fit <- big_spLogReg(G, y01.train=y, dfmax=dfmax, ncores=10)
    cat("done.\n")
    proc.time() - ptm # Stop the clock
  } else {
    ptm <- proc.time() # Start the clock!
    cat(sprintf("Fitting sparse linear regression with %d observations and %d variables... ",
                length(y), ncol(G)))
    lasso.fit <- big_spLinReg(G, y.train=y, dfmax=dfmax, ncores=10)
    cat("done.\n")
    proc.time() - ptm # Stop the clock
  }
  ...
  out.file <- sprintf("%s/simulations/lasso/fold_%s/%s/lasso_%s_%s.txt",
...
#######################################
out.file <- sprintf("%s/simulations/lasso/fold_%s/%s/stats_%s_%s.txt",
                    scratch, fold, resolution, experiment.name, experiment.numb)
