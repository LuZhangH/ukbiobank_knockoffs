...
RUN_PLINK=0
# Whether to perform clumping with PLINK and BH
RUN_PLINK_BH=0
# Whether to perform fine mapping with SUSIE
RUN_SUSIE=1
# Whether to perform fine mapping with CAVIAR
RUN_CAVIAR=0
# Whether to test fixed hypotheses with BH
RUN_BH=0
...
echo "  - Run-PLINK-BH      :" $RUN_PLINK_BH
...
# Screening for BOLT
MODEL_DIR="/scratch/groups/candes/ukbiobank_tmp/simulations/bolt_model"
mkdir -p $MODEL_DIR
mkdir -p $MODEL_DIR"/fold_"$FOLD
BOLT_MODEL=$MODEL_DIR"/fold_"$FOLD"/model_"$EXPERIMENT_NAME"_"$EXPERIMENT_NUMB".txt"

# Output
STATS_DIR="/scratch/groups/candes/ukbiobank_tmp/simulations/bolt"
mkdir -p $STATS_DIR
...
if [ $RUN_PLINK -eq 1 ]; then
  ...
  CLUMP_DIR="/scratch/groups/candes/ukbiobank_tmp/simulations/bolt_clumped"
  mkdir -p $CLUMP_DIR
...
if [ $RUN_PLINK_BH -eq 1 ]; then
  echo "----------------------------------------------------------------------------------------------------"
  echo "Clumping LMM statistics with PLINK and BH"
  echo "----------------------------------------------------------------------------------------------------"
  echo ""
  ...
  #Rscript --vanilla lmm_BH_clumping.R $EXPERIMENT_NAME $EXPERIMENT_NUMB

  CLUMP_THRESHOLD="BH"
  echo "Combining clumping files at threshold $CLUMP_THRESHOLD"
  CLUMP_FILE=$CLUMP_DIR"/stats_"$EXPERIMENT_NAME"_"$EXPERIMENT_NUMB"_BH"
  # Write header
  HEADER=" CHR    F             SNP         BP        P    TOTAL   NSIG    S05    S01   S001  S0001    SP2"
  echo $HEADER > $CLUMP_FILE".clumped"
  # Combine results of clumping into a single file
  ...
  # Remove empty lines
  sed -i '/^$/d' $CLUMP_FILE".clumped"
  echo "Results written on "$CLUMP_FILE".clumped"

else
  echo "----------------------------------------------------------------------------------------------------"
  echo "Skipping clumping of LMM statistics with PLINK + BH"
  echo "----------------------------------------------------------------------------------------------------"
  echo ""
...
if [ $RUN_SUSIE -eq 1 ]; then
  ...
  echo "----------------------------------------------------------------------------------------------------"
  echo ""
  mkdir -p "/scratch/PI/candes/ukbiobank_tmp/simulations/susie/"
  mkdir -p "/scratch/PI/candes/ukbiobank_tmp/simulations/susie/fold_"$FOLD
  for CLUMP_THRESHOLD in "${CLUMP_THRESHOLD_LIST[@]}"; do
...
if [ $RUN_CAVIAR -eq 1 ]; then
  ...
  mkdir -p "/scratch/PI/candes/ukbiobank_tmp/simulations/caviar/fold_"$FOLD
  for CLUMP_THRESHOLD in "${CLUMP_THRESHOLD_LIST[@]}"; do
    echo "Applying fine mapping to results clumped at threshold "$CLUMP_THRESHOLD
...
if [ $RUN_BH -eq 1 ]; then
  echo "----------------------------------------------------------------------------------------------------"
  echo "Testing fixed hypotheses with BH"
  echo "----------------------------------------------------------------------------------------------------"
  echo ""
  mkdir -p "/scratch/PI/candes/ukbiobank_tmp/simulations/BH/"
  mkdir -p "/scratch/PI/candes/ukbiobank_tmp/simulations/BH/fold_"$FOLD
  Rscript --vanilla lmm_BH.R $EXPERIMENT_NAME $EXPERIMENT_NUMB
else
  echo "----------------------------------------------------------------------------------------------------"
  echo "Skipping fixed hypotheses with BH"
  echo "----------------------------------------------------------------------------------------------------"
  echo ""
fi
