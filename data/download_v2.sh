...
DATA_DIR="/home/unix/lu/ukbiobank"
#DATA_DIR="/oak/stanford/groups/candes/ukbiobank"
...
if [ $CHR == 0 ]; then
  CHROMOSOMES=({21..22} "X" "Y" "XY" "MT")
  CHROMOSOMES_HAP=({21..22})
else 
  CHROMOSOMES=($CHR)
  CHROMOSOMES_HAP=($CHR)
fi

#RELEASE=s488295  # New release (not the one we used)
#RELEASE=s488349
...
if [[ ! $BIM_EXIST -eq 1 ]] || [[ $RESET -eq 1 ]]; then
  ...
  for CHR in ${CHROMOSOMES[@]}
  do
    cp $UKBB_DIR"genotype/ukb_snp_chr"$CHR"_v2.bim" $OUT_DIR
...
do
  ...
  if [[ ! -f $OUT_DIR$OUT_FILE ]] || [[ $RESET -eq 1 ]]; then
    ...
    echo "----------------------------------------------------------------------------------------------------"
    #./ukbgene cal -c$CHR -v
    cp $UKBB_DIR"genotype/ukb_cal_chr"$CHR"_v2.bed" $OUT_DIR
    mv $OUT_DIR"ukb_cal_chr"$CHR"_v2.bed" $OUT_DIR$OUT_FILE
  else
    echo ""
    echo "----------------------------------------------------------------------------------------------------"
...
do
  ...
  if [[ ! -f $OUT_DIR$OUT_FILE ]] || [[ $RESET -eq 1 ]]; then
    ...
    echo "----------------------------------------------------------------------------------------------------"
    #./ukbgene hap -c$CHR -v
    cp $UKBB_DIR"imputed_v3/ukb_hap_chr"$CHR"_v2.bgen" $OUT_DIR
    mv $OUT_DIR"ukb_hap_chr"$CHR"_v2.bgen" $OUT_DIR$OUT_FILE
  else
    echo ""
    echo "----------------------------------------------------------------------------------------------------"
...
if [[ ! $BGI_EXIST -eq 1 ]] || [[ $RESET -eq 1 ]]; then
  ...
  for CHR in ${CHROMOSOMES_HAP[@]}
  do
    cp $UKBB_DIR"imputed_v3/ukb_imp_chr"$CHR"_v3.bgen.bgi" $OUT_DIR
    mv $OUT_DIR"ukb_imp_chr"$CHR"_v3.bgen.bgi" $OUT_DIR"ukb_hap_chr"$CHR".bgen.bgi"
  done
  #rm ukb_hap_bgi.tgz
else
  echo ""
...
if [[ ! -f $OUT_DIR$OUT_FILE ]] || [[ $RESET -eq 1 ]]; then
  ...
  cp $UKBB_DIR"genotype/ukb_snp_qc.txt" $OUT_DIR
else
  echo ""
  echo "----------------------------------------------------------------------------------------------------"
