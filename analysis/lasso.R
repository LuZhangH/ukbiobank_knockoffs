...
if(fit.lasso==TRUE) {
  ...
  rds.file <- sprintf("%s/augmented_data_big/ukb_gen_%s_s%s.rds", scratch, resolution, seed)
...
if(seed==0) {
  ...
  scale.file <- sprintf("%s/augmented_data_big/ukb_gen_%s_s%s_scale.txt", scratch, resolution, seed)
}
...
if(fit.lasso==TRUE) {
    ...
    if(use.PCs) {
      if(seed==0) {
          ...
          beta.file <- sprintf("%s/analysis/knockoffs/%s_%s_s%s_lasso_beta.txt",
...
beta.variants <- beta[1:nrow(Variants),]
beta.covariates <- beta[(nrow(Variants)+1):nrow(beta),]
...
##########################
if(use.PCs) {
    out.file <- sprintf("%s/analysis/knockoffs/%s_%s_s%s_lasso.txt", scratch, phenotype, resolution, seed)
} else {
    out.file <- sprintf("%s/analysis/knockoffs/%s_%s_s%s_lasso_nopc.txt", scratch, phenotype, resolution, seed)
}
...
if(use.PCs) {
    stats.file <- sprintf("%s/analysis/knockoffs/%s_%s_s%s_lasso_stats.txt", scratch, phenotype, resolution, seed)
} else {
    stats.file <- sprintf("%s/analysis/knockoffs/%s_%s_s%s_lasso_stats_nopc.txt", scratch, phenotype, resolution, seed)
}
...
Selections <- Stats %>% filter(is.na(R2)|R2<=R2.max) %>% knockoff.filter(fdr=0.1, offset=offset)
...
cat(sprintf("Number of discoveries made by knockoffs (offset %s):\n", offset))
Selections %>% summarise(Discoveries=n())

# Save list of discoveries
if(use.PCs) {
    discoveries.file <- sprintf("%s/discoveries/%s_knockoffs_%s_s%s_offset%s.txt", scratch, phenotype, resolution, seed, offset)
} else {
    discoveries.file <- sprintf("%s/discoveries/%s_knockoffs_%s_s%s_nopc_offset%s.txt", scratch, phenotype, resolution, seed, offset)
}
