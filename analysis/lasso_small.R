...
if(fit.lasso) {
    rds.file <- sprintf("%s/augmented_data_small/ukb_gen_%s.rds", scratch, resolution)
    if(file.exists(rds.file)){
        ...
        tmpdir <- "/scratch/groups/candes/R_tmp"
        tmp.file <- sprintf("%s/augmented_data_small/ukb_gen_%s", scratch, resolution)
    ...
    scale.file <- sprintf("%s/augmented_data_small/ukb_gen_%s_scale.txt", scratch, resolution)
    ...
    Subjects <- obj.bigSNP$fam %>% as_tibble %>%
        transmute(FID=family.ID, IID=sample.ID)
    ...
    if(phenotype.class=="factor") {
        ...
        cat(sprintf("Fitting sparse linear regression with %d observations, %d variants and %d covariates... ",
                    length(y), ncol(G), ncol(covar.train)))
        lasso.fit <- big_spLinReg(G, y.train=y, ind.train=ind.train, covar.train=covar.train, dfmax=dfmax, ncores=10)
    }
    cat("done.\n")
    proc.time() - ptm # Stop the clock
    ...
    # Saving beta matrix
    beta.file <- sprintf("%s/analysis/knockoffs/%s_%s_lasso_fold_%s_beta.txt", scratch, phenotype, resolution, fold)
    ...
    ##########################
    out.file <- sprintf("%s/analysis/knockoffs/%s_%s_fold_%s_lasso.txt", scratch, phenotype, resolution, fold)
    ...
    out.file <- sprintf("%s/analysis/knockoffs/%s_%s_fold_%s_lasso.txt", scratch, phenotype, resolution, fold)
    Lasso.res <- read_delim(out.file, delim=" ") %>% mutate(Z=Importance)
}
...
discoveries.file <- sprintf("%s/discoveries/%s_knockoffs_%s_fold_%s.txt", scratch, phenotype, resolution, fold)
