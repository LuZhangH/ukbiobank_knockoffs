...
#PHENOTYPE_LIST=("height" "sbp" "platelet" "cvd" "diabetes" "hypothyroidism" "respiratory" "glaucoma")
PHENOTYPE_LIST=("glaucoma")

# Slurm parameters
PART=candes,hns       # Partition names
MEMO=80G              # Memory required (80G for BOLT)
TIME=04-0:00:00       # Time required (4d for BOLT)
...
for PHENOTYPE in "${PHENOTYPE_LIST[@]}"; do    
  ...
  if [ ! -s $OUT_FILE ]; then
    # Script to be run
    SCRIPT="analysis_lmm.sh $PHENOTYPE"
