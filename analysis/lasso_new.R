...
Covariates <- Phenotypes %>% select(covariate.names[[1]])
...
if(fit.lasso==TRUE) {
    ...
    beta %>% as.tibble() %>% write_delim(beta.file, delim=" ")
    ...
    # Load the lasso estimates
    out.file <- sprintf("%s/analysis/knockoffs/%s_%s_lasso.txt", scratch, phenotype, resolution)
    Lasso.res <- read_delim(out.file, delim=" ", col_types=cols()) %>% mutate(Z=Importance)
}
...
Selections <- Stats %>% filter(R2<R2.max) %>% knockoff.filter(fdr=0.1, offset=1)
