...
mkdir -p $STATS_DIR
STATS_FILE=$STATS_DIR"/"$PHENOTYPE"_stats.txt"
...
if [ $RUN_PLINK -eq 1 ]; then
  ...
  STATS_FILE_TMP=$CLUMP_DIR"/"$PHENOTYPE"_stats.txt"
  ...
  for CHR in $CHR_LIST; do
    ...
    for CLUMP_THRESHOLD in "${CLUMP_THRESHOLD_LIST[@]}"; do
      CLUMP_FILE=$CLUMP_DIR"/"$PHENOTYPE"_"$CLUMP_THRESHOLD
  ...
  source activate ukb
  CLUMP_THRESHOLD=0.00000005
  Rscript --vanilla summarise_lmm.R $PHENOTYPE $CLUMP_THRESHOLD "00"
