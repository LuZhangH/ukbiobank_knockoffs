...
for(chr in chr.list) {
    ...
    plink.exclude <- "/scratch/PI/candes/ukbiobank_tmp/analysis/variants/ukb_gen_exclude.txt"
    plink.stats <- sprintf("%s/analysis/bolt_clumped/%s_stats_fold_%s.tmp", scratch, phenotype, fold)
    plink.threshold <- sprintf("%s", fdr.threshold)
    clump.dir <- sprintf("/scratch/PI/candes/ukbiobank_tmp/analysis/bolt_clumped")
    plink.out <- sprintf("%s/%s_fold_%s_BH_chr%d", clump.dir, phenotype, fold, chr)
