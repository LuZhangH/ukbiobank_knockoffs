...
pheno.name <- sprintf("Y_a%s_%s", amplitude, 1)
...
# Save results
if(correct.bias) {
    out.file <- sprintf("%s/simulations_small/susie/fold_%s/%s_%s_%s_%s_nobias.txt",
                        scratch, fold, experiment.name, amplitude, clumping, coverage)
} else {
    out.file <- sprintf("%s/simulations_small/susie/fold_%s/%s_%s_%s_%s.txt",
                        scratch, fold, experiment.name, amplitude, clumping, coverage)
}
