...
n.folds <- 10
fold.list <- stringr::str_pad(seq(n.folds),width=2,pad="0")
...
Params <- expand.grid(Amplitude=amplitude.list, Fold=fold.list, Clumping=clumping.list) %>%
    as_tibble() %>%
    mutate(Clumping=as.character(Clumping), Clumping.num=as.numeric(Clumping), Fold=as.character(Fold))
...
calibrate_oracle <- function(fdr, power, fdr.target=0.1) {
    ...
    if(all(is.na(power))||all(is.nan(power))) {
        return(NA)
    }
    power <- c(0,power)
    fdr <- c(0,fdr)
...
Selections.loci <- Selections %>%
    ...
    ungroup()
Selections.loci$Size[is.na(Selections.loci$SNP.lead)] <- NA
...
Distinct.summary <- Selections.loci %>% group_by(Amplitude, H2, Fold, Clumping) %>%
    ...
    mutate(Method="LMM") %>%
    mutate(FDP=replace_na(FDP,0))

Oracle <- Distinct.summary %>%
    group_by(Method, Amplitude, H2, Clumping) %>%
    summarise(FDP=mean(FDP), BP.width=mean(BP.width,na.rm=T), Size=mean(Size,na.rm=T),
    ...
    summarise(Discoveries=calibrate_oracle(FDP, Discoveries),
              ...
              FDP=0.1, Clumping="oracle", Fold="00")
Distinct.summary <- rbind(Distinct.summary, Oracle)
Distinct.summary$Size[is.nan(Distinct.summary$Size)] <- NA
Distinct.summary$BP.width[is.nan(Distinct.summary$BP.width)] <- NA

cat("Summary of distinct discoveries with LMM:\n")
Distinct.summary %>% print()

out.file <- sprintf("%s/simulations_small/summary/lmm_%s_distinct.txt", scratch, experiment.name)
...
Distinct.consolidated.summary$Size[is.nan(Distinct.consolidated.summary$Size)] <- NA
Distinct.consolidated.summary$BP.width[is.nan(Distinct.consolidated.summary$BP.width)] <- NA

cat("Summary of distinct consolidated discoveries with LMM:\n")
Distinct.consolidated.summary %>% print()

out.file <- sprintf("%s/simulations_small/summary/lmm_%s_distinct_consolidated.txt", scratch, experiment.name)
...
out.file <- sprintf("%s/simulations_small/summary/lmm_%s_loci.txt", scratch, experiment.name)
