...
Selections.susie <- foreach(idx = 1:nrow(Params), .combine = 'rbind') %dopar% {
    ...
    if(susie.nobias) {
        fine.file <- sprintf("%s/simulations_small/susie/fold_%s/%s_%s_%s_%s_nobias.txt",
                             scratch, fold, experiment.name, amplitude, clumping, coverage)
    } else {
        fine.file <- sprintf("%s/simulations_small/susie/fold_%s/%s_%s_%s_%s.txt",
                             scratch, fold, experiment.name, amplitude, clumping, coverage)
    }
...
if(susie.nobias) {
    out.file <- sprintf("%s/simulations_small/summary/lmm_%s_susie_nobias.txt", scratch, experiment.name)
} else {
    out.file <- sprintf("%s/simulations_small/summary/lmm_%s_susie.txt", scratch, experiment.name)
}
...
load.caviar <- function(amplitude, fold, clumping, coverage=0.9) {
    posterior.file <- sprintf("%s/simulations_small/caviar/fold_%s/%s_%s_%s.txt",
    ...
    if(nrow(Posterior)>0) {
        Posterior <- Posterior %>% select(CHR, SNP.lead, BP.lead, SNP, BP, P.set, P.causal, Causal)
...
out.file <- sprintf("%s/simulations_small/summary/lmm_%s_caviar.txt", scratch, experiment.name)
