...
source("../simulations/utils_outer.R")
...
#fold.list <- stringr::str_pad(c(7,8),width=2,pad="0")
...
Selections <- foreach(idx = 1:nrow(Params), .combine = 'rbind') %dopar% {
    ...
    res.file <- sprintf("%s/simulations_small/lasso/fold_%s/%s/lasso_%s_%s.txt",
...
Selections %>% filter(Resolution=="Radj2") %>%
    ...
    summarise(True=sum(True)/n.folds, FDP=sum(FDP)/n.folds)
...
Selections.outer %>% group_by(Amplitude, Fold) %>%
    summarise(N=n(), True=sum(Causal), False=sum(!Causal), FDP=mean(!Causal)) %>%
    ungroup() %>%
    group_by(Amplitude) %>%
    summarise(True=sum(True)/n.folds, FDP=sum(FDP)/n.folds) %>% print()
...
na.replace <- function(x, na.value=0) {
    x[is.na(x)] <- na.value
    return(x)
}
...
Distinct.summary %>% group_by(Amplitude, H2, Resolution) %>%
    mutate(FDP=na.replace(FDP)) %>%
    summarise(Discoveries.true=mean(Discoveries.true), FDP=mean(FDP)) %>%
    filter(Resolution=="Radj2") %>% print()

out.file <- sprintf("%s/simulations_small/summary/knockoffs_%s_distinct.txt", scratch, experiment.name)
...
out.file <- sprintf("%s/simulations_small/summary/knockoffs_%s_distinct_consolidated.txt", scratch, experiment.name)
...
Cluster.summary <- foreach(idx = 1:nrow(Params), .combine = 'rbind') %dopar% {
    ...
    fold <- Params$Fold[idx]
    resolution <- Params$Resolution[idx]
    Loci <- Selections.loci.consolidated %>% filter(Amplitude==amplitude, Resolution==resolution, Fold==fold)
...
out.file <- sprintf("%s/simulations_small/summary/knockoffs_%s_loci.txt", scratch, experiment.name)
